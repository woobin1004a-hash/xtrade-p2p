        ...
        /**
         * 영수증 PNG: 모달 DOM을 html2canvas로 캡처 → Edge `receipt-png-upload`가 Storage에 올리고 서명 URL 반환
         * → Telegram.WebApp.downloadFile (Blob 직접 저장은 웹뷰에서 불안정)
         */
        async function downloadCurrentTransactionReceipt() {
            ...
            if (!body || !String(body.innerHTML || '').trim()) {
                var missB = langText('영수증 내용이 없습니다.', 'Receipt is empty.');
            ...
            var exportedLine =
                ...
                ': ' +
                new Date().toLocaleString(uiLangMode === 'en' ? 'en-US' : 'ko-KR', {
                    dateStyle: 'medium',
                    timeStyle: 'short',
                }) +
                ' · XTrade P2P';
            ...
            try {
                ...
                if (dlBtn) dlBtn.textContent = langText('업로드 중...', 'Uploading...');

                var fd = new FormData();
                fd.append('order_id', String(id));
                fd.append('file', blob, 'receipt.png');

                var hdr = supabaseHeaders({ 'X-Telegram-User-Id': telegramUserId });
                delete hdr['Content-Type'];
                delete hdr['content-type'];

                var uploadUrl = baseUrl + '/functions/v1/receipt-png-upload';
                ...
                if (!res.ok) {
                    ...
                    var errMsg =
                        data && data.error
                            ? String(data.error)
                            : langText('서버 업로드에 실패했습니다.', 'Upload failed.') + ' (HTTP ' + res.status + ')';
                ...
                function showReceiptPngSaveOverlay(pngBlob, name, publicHttpsUrl) {
                    ...
                    var httpsUrl = publicHttpsUrl ? String(publicHttpsUrl).trim() : '';
                    ...
                    saveBtn.type = 'button';
                    saveBtn.textContent = langText('PNG 저장하기', 'Save PNG');
                    saveBtn.style.cssText =
                        'display:inline-block;padding:14px 28px;background:#6c5ce7;color:#fff;border:none;border-radius:8px;font-weight:600;font-size:16px;cursor:pointer;';
                    ...
                    function runPngSaveFallbacks() {
                        // Web Share (일부 환경에서 파일 저장으로 이어짐)
                        if (typeof navigator !== 'undefined' && navigator.share && typeof File !== 'undefined') {
                            try {
                                var shareFile = new File([pngBlob], fname, { type: 'image/png' });
                                if (typeof navigator.canShare === 'function' && !navigator.canShare({ files: [shareFile] })) {
                                    afterShareFail();
                                    return;
                                }
                                navigator.share({ files: [shareFile], title: fname }).then(
                                    function () {
                                        cleanup();
                                    },
                                    function () {
                ...
                /** PC·웹: 오버레이 + 클릭 시 저장 API(공개 URL은 대체 경로용으로 전달) */
                function runPcReceiptSave(pngBlob, fname, publicHttpsUrl) {
                    showReceiptPngSaveOverlay(pngBlob, fname, publicHttpsUrl);
                }
                ...
                var net =
                    langText('PNG를 만들거나 올리지 못했습니다.', 'Could not create or upload PNG.') +
                    (detail ? ' (' + detail + ')' : '');
